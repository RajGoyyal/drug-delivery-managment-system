...
INSERT_TXN = 'INSERT INTO inventory_transactions(drug_id,delta,reason) VALUES(?,?,?)'
...
def list_patients():
    conn=get_conn(); cur=conn.execute('SELECT id,name,age,contact FROM patients ORDER BY id'); data=rows(cur); conn.close(); return jsonify(data)

@app.post('/api/patients')
def create_patient():
    p=request.get_json(force=True)
    name=p.get('name','').strip(); age=p.get('age'); contact=p.get('contact')
...
@app.get('/api/drugs')
def list_drugs():
    conn=get_conn(); cur=conn.execute('SELECT id,name,dosage,frequency,stock,reorder_level FROM drugs ORDER BY id'); data=rows(cur); conn.close(); return jsonify(data)
...
def delete_drug(drug_id: int):
    ...
    conn.close();
    if not deleted: return jsonify(NOT_FOUND),404
    return jsonify({'deleted':True})

# --- Deliveries -------------------------------------------------------------
@app.get('/api/deliveries')
def list_deliveries():
    conn=get_conn(); cur=conn.execute("SELECT id, patient_id, drug_id, delivery_date AS scheduled_for, status, 1 AS quantity, NULL AS notes FROM delivery_logs ORDER BY id DESC"); data=rows(cur); conn.close(); return jsonify(data)
...
def update_delivery_status(delivery_id: int):
    ...
    with conn:
        ...
        if cur.rowcount==0:
            conn.close(); return jsonify(NOT_FOUND),404
...
def inventory_summary():
    ...
    data=rows(cur); conn.close(); return jsonify(data)

@app.get('/api/inventory/transactions')
def inventory_transactions():
    limit=int(request.args.get('limit',300))
    conn=get_conn(); cur=conn.execute('SELECT id,drug_id,delta,reason,created_at FROM inventory_transactions ORDER BY id DESC LIMIT ?',(limit,)); data=rows(cur); conn.close(); return jsonify(data)
...
def inventory_adjust():
    ...
    with conn:
        ...
        if not row: conn.close(); return jsonify(NOT_FOUND),404
        new_stock=max(0,(row[0] or 0)+delta)
        conn.execute('UPDATE drugs SET stock=? WHERE id=?',(new_stock,drug_id))
        conn.execute(INSERT_TXN,(drug_id,delta,reason))
    conn.close(); return jsonify({'ok':True})
...
def create_batch():
    ...
    with conn:
        ...
        conn.execute(INSERT_TXN,(drug_id,qty,f"batch:{p.get('batch_no') or ''}"))
...
def list_batches():
    ...
    params.append(limit)
    conn=get_conn(); cur=conn.execute('SELECT id,drug_id,batch_no,isbn,producer,transporter,mfg_date,exp_date,quantity,notes,created_at FROM drug_batches'+where+' ORDER BY id DESC LIMIT ?',params); data=rows(cur); conn.close(); return jsonify(data)
...
def create_removal():
    ...
    with conn:
        ...
        conn.execute(INSERT_TXN,(drug_id,-qty,f'remove:{reason}'))
    conn.close(); return jsonify({'ok':True}),201
...
def list_removals():
    ...
    conn=get_conn(); cur=conn.execute('SELECT id,drug_id,batch_no,reason,quantity,notes,created_at FROM drug_removals'+where+' ORDER BY id DESC LIMIT ?',params); data=rows(cur); conn.close(); return jsonify(data)
